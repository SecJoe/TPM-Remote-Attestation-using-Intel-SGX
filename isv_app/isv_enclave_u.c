...
static sgx_status_t SGX_CDECL isv_enclave_PrintError(void* pms)
{
	ms_PrintError_t* ms = SGX_CAST(ms_PrintError_t*, pms);
	PrintError((const char*)ms->ms_str);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL isv_enclave_PrintError_array(void* pms)
{
	ms_PrintError_array_t* ms = SGX_CAST(ms_PrintError_array_t*, pms);
	PrintError_array(ms->ms_array);
...
static const struct {
	...
	11,
	{
		(void*)isv_enclave_PrintError,
		(void*)isv_enclave_PrintError_array,
...
sgx_status_t secure_verify(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, tpm_enc_att_state_request_message_t* request_msg, uint32_t request_msg_size, sgx_ec256_public_t* tpm_public_key, uint8_t* tpm_attest_data, uint32_t tpm_attest_data_size, sgx_ec256_signature_t* tpm_signature, uint32_t tpm_pcr_size, tpm_enc_att_state_response_message_t* p_ver_msg_out, uint32_t ver_msg_size)
{
	sgx_status_t status;
	ms_secure_verify_t ms;
	ms.ms_context = context;
	ms.ms_request_msg = request_msg;
	ms.ms_request_msg_size = request_msg_size;
	ms.ms_tpm_public_key = tpm_public_key;
	ms.ms_tpm_attest_data = tpm_attest_data;
	ms.ms_tpm_attest_data_size = tpm_attest_data_size;
	ms.ms_tpm_signature = tpm_signature;
	ms.ms_tpm_pcr_size = tpm_pcr_size;
	ms.ms_p_ver_msg_out = p_ver_msg_out;
	ms.ms_ver_msg_size = ver_msg_size;
