...
- On top of that I have implemented enclave which verfiies the Quote (check the Signature) and
  ...
  author. In the implemention of this project I had to solve some additional milestones like: 
				...
				b) The standard SGX crypto library does not support RSA so I had to use ECC for the TPM
	   			 Quotes.
				c) There is no library for unstusted code (service-provider aka. remote party) to 					 decrypt AES packets. So later in the process when I report the verification state 						 to the remote party I had to decrypt the AES packet with OpenSSL.
...
- After that I check some requirements like
				...
				b) limitations regarding the used signature algorithm by the TPM. SGX does not support 						 RSA in the standard sgx_tcypto library. So is had to use Elliptic Curve Digital 						 Signature	Algorithm (ECDSA) in the TPM Quotes. Also SGX supprts only the specific 					 ECC curves (secp256r1 also called NIST P-256, P-256 or prime256v1). I will check 					 that requirements too.
				c)  ...................
